    ...
    if ( $stmt ) 
      ...
      $stmt->bind_result( $tag, $serial, $makemodel, $os, $hostname, $department, $offcampus, $location );
			...
			printf( "<table id='custom_table' class='tablesorter' summary='Search Results'>
							 	 <thead>
									 <tr>
										 ...
										 <th scope='col' class='header'><div>Property Tag <i class='fa'></i></div></th>
										 <th scope='col' class='header'><div>Serial Number <i class='fa fa-unsorted'></i></div></th>
										 <th scope='col' class='header'><div>Make & Model <i class='fa fa-unsorted'></i></div></th>
										 <th scope='col' class='header'><div>Location <i class='fa fa-unsorted'></i></div></th>
										 <th scope='col' class='header'><div>Department <i class='fa fa-unsorted'></i></div></th>
										 <th scope='col' class='header'><div>Users <i class='fa fa-unsorted'></i></div></th>
									 </tr>
								 </thead>
								 <tbody>" );
			...
			while ( $stmt->fetch() )
			{
				...
				printf( "<tr class='more_info expand-child' style='display:none;'><td></td><td style='border: 2px solid blue' colspan='6'>testing testing testing</td></tr>" );
			}
			...
			printf( "</tbody></table><script>$( '#custom_table' ).tablesorter();
																			 ...
																			 $( '.expand_all' ).click( function(){
																				 ...
																				 if ( $( this ).hasClass( 'closed' ) )
																				   $( '.more_info' ).show(); });
																			 $( 'th.headerSortDown > div > i' ).addClass( 'fa-arrow-down' );
																			 $( 'th.headerSortUp >  div > i' ).addClass( 'fa-arrow-up' );
</script>" );
    }
	}
